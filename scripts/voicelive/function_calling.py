...
# -------------------------------------------------------------------------
# https://learn.microsoft.com/ja-jp/azure/ai-services/speech-service/how-to-voice-live-function-calling
...
assert load_dotenv(override=True), "Failed to load environment variables from .env file"
...
class AsyncFunctionCallingClient:
    ...
    def __init__(
        ...
        self.available_functions: dict[str, Callable[[str | Mapping[str, Any]], Mapping[str, Any]]] = {
    ...
    async def _handle_event(self, event):
        ...
        if event.type == ServerEventType.SESSION_UPDATED:
            ...
            if event.item.type == ItemType.FUNCTION_CALL:
                ...
                logger.info(
                    f"Function call detected: {function_call_item.name} with call_id: {function_call_item.call_id}"
                )
    ...
    def get_current_time(self, arguments: str | Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    def get_current_weather(self, arguments: str | Mapping[str, Any]):
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            ...
            "If you are asked about the weather, please respond with 'I will get the weather for you. Please wait a moment.' and then call the get_current_weather function. "  # noqa: E501
            "If you are asked about the time, please respond with 'I will get the time for you. Please wait a moment.' and then call the get_current_time function. "  # noqa: E501
            "Explain when you're using a function and include the results in your response naturally. Always start the conversation in English.",  # noqa: E501
        ),
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        input_devices = [
            ...
            if cast(int | float, p.get_device_info_by_index(i).get("maxInputChannels", 0) or 0) > 0
        ]
        # Check for output devices
        output_devices = [
            i
            for i in range(p.get_device_count())
            if cast(int | float, p.get_device_info_by_index(i).get("maxOutputChannels", 0) or 0) > 0
        ]
        p.terminate()
